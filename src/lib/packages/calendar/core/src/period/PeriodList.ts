...
export class PeriodList {
    ...
    findExcludeInRange(cell:Cell) {
        ...
        return !allStart.every(start => { //Проверяем, что нет "разрывов" между режимами работы
            return allEnd.findIndex(_end => {
    ...
    findPeriodsInRange(cell:Cell) { //Ищем периоды под каждый день
    ...
    isExcludeLite(cellOrDate:Cell | Date) { //Простая версия для проверки. Подходит для 1 слота и для даты
    ...
    isExclude(cellOrDate:Cell | Date) { //Автоматическая версия для проверки
